...
int	op_collect_cmd(t_operation **operations, size_t i, t_list *lst)
{
	...
	while (lst && !check_op_type(lst))
	{
		...
		{
			operations[i]->cmd = ft_strdup(((
							t_token *)lst->content)->token_word);
			if (!operations[i]->cmd)
				return (0);
			break ;
		}
...
int	op_data_collector(t_operation **operations, size_t i, t_list *lst)
{
	if (check_op_type(lst))
		lst = lst->next;
	if (!op_collect_cmd(operations, i, lst) || !op_prep_redirections(
			operations[i], lst))
	{
		free_operations(operations);
		return (0);
	}
	if (!op_get_redirections(operations[i], lst) || !op_prep_args(
			operations[i], lst))
	{
		free_operations(operations);
		return (0);
